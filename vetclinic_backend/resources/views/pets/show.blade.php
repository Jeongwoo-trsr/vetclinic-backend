...
<div class="max-w-4xl mx-auto">
    <div class="flex justify-between items-center mb-6">
        ...
        <div class="flex gap-2">
            <a href="{{ route('pets.edit', $pet->id) }}" class="px-4 py-2 bg-[#d4931d] text-white rounded hover:bg-[#fcd34d] hover:text-[#2d3748]">
                <i class="fas fa-edit mr-2"></i>Edit
            </a>
            <a href="{{ Auth::user()->role === 'admin' ? route('admin.pets') : route('pet-owner.pets') }}" class="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-400">
       ...
       <!-- Header -->
        <div class="bg-[#1e3a5f] px-8 py-8">
            <div>
                ...
                <p class="text-gray-200 text-lg">{{ $pet->species }} - {{ $pet->breed ?? 'Mixed' }}</p>
            </div>
        </div>
                ...
                <div class="bg-gray-50 p-4 rounded-lg">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <p class="font-medium">{{ $pet->owner->user->name }}</p>
                        </div>
                        <div>
                            <label class="text-sm text-gray-600">Email</label>
                            <p class="font-medium">{{ $pet->owner->user->email }}</p>
                        </div>
                        <div>
                            <label class="text-sm text-gray-600">Phone</label>
                            <p class="font-medium">{{ $pet->owner->user->phone ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="text-sm text-gray-600">Emergency Contact</label>
                            <p class="font-medium">{{ $pet->owner->emergency_contact ?? 'N/A' }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="bg-gray-50 p-3 rounded flex justify-between items-center">
                        ...
                        <span class="px-3 py-1 rounded text-xs font-semibold
                            ...
                            @elseif($appointment->status === 'scheduled') bg-[#0066cc] bg-opacity-20 text-[#003d82]
                            @else bg-[#fcd34d] bg-opacity-30 text-[#2d3748]
                            @endif">
